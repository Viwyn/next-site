...
const DraggableFloatingDiv: React.FC<DraggableFloatingDivProps> = ({ children, initialX = 50, initialY = 50, isVisible}) => {
    ...
    return (
        <div ref={divRef} className={`${isDragging ? 'cursor-grabbing' : 'cursor-grab'} fixed z-99 bg-gray-700 text-black dark:text-white rounded-lg border-2 border-black dark:border:white shadow-xl select-none transition-colors flex flex-col ${isHidden ? 'invisible' : ''}`} style={{ left: position.x, top: position.y }} onMouseDown={handleMouseDown}>
